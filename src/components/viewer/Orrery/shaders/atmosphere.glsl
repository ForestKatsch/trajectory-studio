...
vec3 atmosphereColor(vec3 position, vec3 direction, vec3 dir_star, vec3 color_star, vec4 param_atmosphere, vec4 param_raleigh) {
  ...
  if(ray_planetView.y > 0.0) {
    length_viewRay = (ray_planetView.x - ray_atmosphereView.x) * 0.99;
  }

  // The total atmosphere color visible at `position`.
  vec3 color = vec3(0.0);

  float length_stepSize = length_viewRay / float(SCATTERING_STEPS);
  
  for(int i=0; i<=SCATTERING_STEPS; i++) {
    
    // The point in the atmosphere, in model space.
    vec3 pos_sample = pos_viewRayStart + direction * length_stepSize * float(i);
